...
export const UpdateProduct = async (
	...
	const code_productResult = props.product_code !== undefined ? wrapType<ValidString, InvalidStringException>(
		() => ValidString.from( props.product_code! ) ) : product.product_code

	if ( code_productResult instanceof BaseException ) {
		errors.push( new InvalidStringException( 'code_product' ) )
	}

	const nameResult = props.name !== undefined ? wrapType<ValidString, InvalidStringException>(
		() => ValidString.from( props.name! ) ) : product.name

	if ( nameResult instanceof BaseException ) {
		errors.push( new InvalidStringException( 'name' ) )
	}

	const descriptionResult = props.description !== undefined ? wrapType<ValidString, InvalidStringException>(
		() => ValidString.from( props.description! ) ) : product.description

	if ( descriptionResult instanceof BaseException ) {
		errors.push( new InvalidStringException( 'description' ) )
	}

	const brandResult = props.brand !== undefined ? wrapType<ValidString, InvalidStringException>(
		() => ValidString.from( props.brand! ) ) : product.brand

	if ( brandResult instanceof BaseException ) {
		errors.push( new InvalidStringException( 'brand' ) )
	}

	const priceResult = props.price !== undefined ? wrapType<ValidInteger, InvalidIntegerException>(
		() => ValidInteger.from( props.price! ) ) : product.price

	if ( priceResult instanceof BaseException ) {
		errors.push( new InvalidIntegerException( 'price' ) )
	}

	const image_urlResult = props.image_url !== undefined ? wrapType<ValidURL, InvalidURLException>(
		() => ValidURL.from( props.image_url! ) ) : product.image_url

	if ( image_urlResult instanceof BaseException ) {
		errors.push( new InvalidURLException( 'image' ) )
	}

	const stockResult = props.stock !== undefined ? wrapType<ValidInteger, InvalidIntegerException>(
		() => ValidInteger.from( props.stock! ) ) : product.stock

	if ( stockResult instanceof BaseException ) {
		errors.push( new InvalidIntegerException( 'stock' ) )
	}

	const average_rankResult = props.average_rank !== undefined ? wrapType<ValidRank, InvalidRankException>(
		() => ValidRank.from( props.average_rank! ) ) : product.average_rank

	if ( average_rankResult instanceof BaseException ) {
		errors.push( new InvalidRankException( 'rank' ) )
	}

	const category_nameResult = props.category_name !== undefined ? wrapType<ValidString, InvalidStringException>(
		() => ValidString.from( props.category_name! ) ) : product.category

	if ( category_nameResult instanceof BaseException ) {
		errors.push( new InvalidStringException( 'category_name' ) )
	}

	const discountResult = props.discount !== undefined ? wrapType<UUID, InvalidUUIDException>(
		() => UUID.from( props.discount! ) ) : product.discount
