...
export function paymentToJson(payment: Payment): Record<string, any> {
    return {
        id: payment.id.value,
        creationDate: payment.creationDate.value,
        approved: payment.approved.value,
        deliveryName: payment.deliveryName.value,
...
export function paymentFromJson (json: Record<string, any>): Payment | BaseException[] {
    ...
    const approved = wrapType<ValidBoolean, InvalidBooleanException>(
        () => ValidBoolean.from(json.approved))
